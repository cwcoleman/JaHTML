		...
		<div class="container">
			...
			<form name="myForm" method="post" action="order.php?orderId=<?php echo $orderId; ?>">
				<div class="in-form">
					...
					?>
						Order #: <a href="KB.php?orderId=<?php echo $orderId; ?>">Related Issues</a><input type="text" name="orderId" value="<?php echo $row['orderId']; ?>" required="" 
						<?php if(isset($_SESSION['role']) && $_SESSION['role'] == "admin") { echo ""; }
						else {echo "readonly='readonly'";} ?> > <!-- onfocus="this.value = '';" onblur="if (this.value == '') {this.value = 'Order Description';}"-->
                        <br>
						<br>
						<br>
						Maintenace Description:<input type="text" name="desc_prob" value="<?php echo $row['desc_prob']; ?>" required="">
                        <br>
						<br>
						<br>
						Closest Branch<input type="text" name="close_branch" value="<?php echo $row['close_branch']; ?>" required="">
                        <br>
						<br>
						<br>
						Assigned to:<input type="text" name="techId" value="<?php echo $row['techId']; ?>" required="" <?php if(isset($_SESSION['role']) && $_SESSION['role'] == "admin") { echo ""; }
						else {echo "readonly='readonly'";} ?>>
                        <br>
						<br>
						<br>
						Status:<input type="text" name="status" value="<?php echo $row['status']; ?>" required="" <?php if(isset($_SESSION['role']) && ($_SESSION['role'] == "admin") || $_SESSION['role'] == "technician") { echo ""; }
						else {echo "readonly='readonly'";} ?>>
                        <br>
						<br>
						<br>
						Uploaded Picture:<?php if($row['prob_picture']!=""){ ?><a href="images/<?php echo $row['prob_picture']; ?>"><img src="upload_pic/<?php echo $row['prob_picture']; ?>" height="180" widht="150"></a><?php } else { echo " No Picture"; } ?>
                        <br>
						<br>
						<br>
                        <h4>Comments</h4>
						<table border="0">
						<?php
						$SQLcmnt = "Select * from comments where orderId='$orderId'";
						$resultCmnt = mysqli_query($link, $SQLcmnt);
						while($row=mysqli_fetch_array($resultCmnt, MYSQLI_BOTH))
						{
						?>
						<tr>
							<td><?php echo $row['comment'] . "<br />By - " . $row['cmtBy'] . "  -  " . $row['timeStamp'] . "<br /><br />"; ?></td>
						</tr>
						<?php
						}
						?>
